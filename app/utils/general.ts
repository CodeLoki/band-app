...
export function sortBy<T extends DocumentSnapshot>(
	models: T[],
	...
	return models.sort((a: T, b: T) => {
		...
		if (!aData || !bData) {
			throw new Error(
				logError("[sortBy] Model data not present", models, key, aData, bData),
			);
		}
...
export const CardStyle =
	"card bg-base-200 card-border border-blue-400/30 shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-200 cursor-pointer w-full";
